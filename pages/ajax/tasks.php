...
<div class="dataTable_wrapper">
    <table class="table table-striped table-bordered table-hover" id="dataTables-example">
        ...
        <tbody>
            <?php
            $i = 0;
            $result = mysqli_query($con,"SELECT * FROM custom_tasks WHERE status = 0 ORDER BY id DESC");
                    ...
                    <td dir="ltr">
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                <li>
                                    <a href="javascript:void(0);" onclick="re_send(<?php echo $row['id']; ?>);">ارسال فوری</a>
                                </li>
                                <li>
            ...
            $result = mysqli_query($con,"SELECT * FROM tasks WHERE status = 0 ORDER BY id DESC");
            while($row = mysqli_fetch_array($result))
            {
                $i += 1;
            ...
            ?>
            <?php
            $result = mysqli_query($con,"SELECT * FROM strategy_tasks WHERE status = 0 ORDER BY id DESC");
            while($row = mysqli_fetch_array($result))
            {
                $i += 1;
                $contact_id = $row['contact_id'];
